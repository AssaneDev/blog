...
class AuthDB
{
    ...
    public function login(string $userId): void
    {
        ...
        $signature = hash_hmac("sha256", $sessionId, "Hakkuframwork the bosse");
    ...
    public function isLoggin()
    {
        ...
        if ($sessionId && $signature) {
            $hash = hash_hmac("sha256", $sessionId, "Hakkuframwork the bosse");
            $hash_math = hash_equals($hash, $signature);
            if ($hash_math) {
